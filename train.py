...
from hrnet.getAttributeFeature import load_model
...
def train(cfg):
    ...
    if cfg.eval_interval > 0:
        gallery_loader, query_loader = get_test_loader(dataset=cfg.dataset,
                                                       ...
                                                       image_size=cfg.image_size,
                                                       num_workers=4)
    model_attr = load_model()
    model_attr = torch.nn.DataParallel(model_attr).cuda()
    # model
    model = Baseline(model_attr = model_attr,
